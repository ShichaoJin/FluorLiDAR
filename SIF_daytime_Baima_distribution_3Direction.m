...
filepath_SIF=strcat(root,'\PAR2SIF\'); %  SIF data directory; 输出点云保存路径

Files=dir([filepath_PAR]); %PAR directory, 辐射输入点云
...
for i = 1:length(V)  % assign values to variables using string matching
    ...
    if isempty(j) || isnan(N(j+1))                       % N is not None and J is not empty;  N的值既不是nan,J的值也不为空
        if i==2
...
%leaf biochemistry
vi = ones(length(V),1);
leafbio.Cab = 40;   % Chlorophyll content; 叶绿素浓度
