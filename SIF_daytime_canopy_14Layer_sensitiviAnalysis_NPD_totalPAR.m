...
%filepath='E:\DiffusePAR\4Layer_30Points\'; %D:\DiffusePAR\18Layer_30Points\
%Files=dir(['E:\DiffusePAR\4Layer_30Points\*.txt']); % data directory

workdir = "\\Jinlab309\d\JinShichao\3DSIF\Malixia_14Layer30Points"; 
densities = ["raw","0.01", "0.05", "0.1", "0.15", "0.2", "0.25", "0.3", "0.35", "0.4", "0.45", "0.5", "0.55", "0.6"]; %"raw","0.01", "0.05",

for density = densities
    filepath = strcat(workdir,'\SIF_Malixia\',density,'\');
    Files=dir([sprintf('%s\\*.txt',filepath)]);
    
    RT_ms=load('RT_ms3.txt');
    RZT_ms=load('RZT_ms2.txt');
    %leafindex=load('leafindex_JSC.txt'); % 由于原始数据中存在重复点，我处理的数据与原始的leafindex不一样，此处用自己生成的
    leafindex=load(sprintf('%s\\leafindex\\%s\\leafindex.txt',workdir, density)); 
    number=length(Files);
    ...
    MbI          = leafopt.MbI;  %backward scattering fluorescence matrix, I for PSI and II for PSII
    MbII         = leafopt.MbII;
    MfI          = leafopt.MfI;  %forward scattering fluorescence matrix,  I for PSI and II for PSII
    MfII         = leafopt.MfII;
    ...
    for daytime=1:number
        ...
        %dlmwrite('D:\\filename4layer.txt',LiDARdata,'delimiter',' ');
        ...
        for i=1:length(PAR)
            ...
            %sifdistribution(i,4)=(MpluEsunI(98)+MpluEsunII(98)+MpluEpluI(98)+MpluEpluII(98))/pi;%760nm   %  直接激发的荧光部分，只计算了后向plu激发的荧光？           
            sifdistribution(i,4)=(MpluEsunI(98)+MpluEsunII(98)+MpluEpluI(98)+MpluEpluII(98))/pi + (MminEsunI(98)+MminEsunII(98)+MminEpluI(98)+MminEpluII(98))/pi;    % 加号后半部分是否应该加上？ 还是说在SIF_MS中考虑了，不需要加？               %Shichao
    ...
    filename_path=strcat('E:\NJU\3DSIF_Zch\Figure5-8CanopySIF_vs_SimulatedSIF_newData_totalPAR_reviseLeafSIF\','SimulatedSIF_14Layer_30Points_',density,'.txt')
    %eval(['save ' filename_path 'sif_canopy', '-ASCII']) %保存SIF点云
    save(filename_path, "sif_canopy", "-ascii")
end
